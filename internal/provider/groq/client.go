...
func (c *GroqClient) callWithClient(reqBody schema.RequestOpenAICompatable) ([]byte, error) {
...
// GroqClient implements the Provider interface for Groq
...
// NewGroqClient creates a new GroqClient with the provided configuration
func NewGroqClient(providerURL, model, token string, config schema.HttpClientConfig) *GroqClient {
	return &GroqClient{
		ProviderURL: providerURL,
		Model:       model,
		Token:       token,
		httpClient:  client.NewHttpClient(config),
		throttler:   NewThrottler(3 * time.Second),
	}
}

// Call implements the Provider interface
func (c *GroqClient) Call(ctx context.Context, payload []byte, path string) ([]byte, error) {
